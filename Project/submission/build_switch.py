...
archive_hash = 'b1a9d015af288665ca5c7d646a413b411694dcf3a3c92edaf6c06d2eb39145d0'
...
with tarfile.open(archive) as bundle:
    original = (
        bundle.extractfile('openvswitch-3.3.9/lib/mac-learning.c').read().decode()
    )
global_policy = r"""static void
...
updated = updated.replace(
    marker,(ROOT / 'fair_controller.inc').read_text() + '\n' + marker
)
capacity_check = '        if (hmap_count(&ml->table) >= ml->max_entries) {'
if updated.count(capacity_check) != 1:
    raise RuntimeError('Unexpected capacity check')
updated = updated.replace(
    capacity_check,'        evict_before_learning(ml);\n\n' + capacity_check
)
updated = updated.replace(
...
with(SOURCE.parent / 'build.log').open('w') as log:
    configure = [
        './configure',
        '--prefix=/opt/mac-flood/install',
        '--localstatedir=/opt/mac-flood/var',
        '--sysconfdir=/opt/mac-flood/etc',
        '--disable-ssl',
        '--disable-libcapng',
        'CFLAGS=-O2 -g',
    ]

    commands = ([] if(SOURCE / 'Makefile').exists() else [configure]) + [
        ['make','-j4']
    ]
    for command in commands:
        subprocess.run(
            command,cwd=SOURCE,stdout=log,stderr=subprocess.STDOUT,check=True
        )
