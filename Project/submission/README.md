...
## Interactive message demonstration

```sh
sudo python3 run_live.py
```

Use the same build and Ubuntu dependencies as the automatic demo below. No rebuild is required just for adding this script.

At each of Baseline, After attack and Recovery, enter a message to transmit from Alice to Bob. You can send multiple messages. Enter `/next` to advance or `/quit` to stop. The script repeats all phases with the custom defense enabled. It prints actual matching receptions at Bob and the attacker, then a one-message summary. Without defense, both should receive your message after flooding; in baseline, recovery and every defended phase, only Bob should receive it.

Messages may contain at most 38 UTF-8 bytes. Each frame has a four-byte LIVE marker and a four-byte sequence number, leaving 38 bytes of the existing 46-byte payload for your text. It remains a 60-byte Ethernet frame excluding FCS, with Alice as source, Bob as destination and EtherType 0x88B5. Both receivers start before transmission; reception is matched against the entire frame. Receivers observe for up to three seconds per message. Their displayed order can vary with process scheduling.

Each case has a four-minute deadline, including setup, tests and interaction, with a small margin reserved for completion. The remaining time is displayed at every prompt. If it expires, the script stops and cleans up instead of letting ordinary MAC aging invalidate the experiment. The switch retains the report's 300-second aging interval. Start again for a fresh case; Bob is not kept alive artificially during the attack.

Output goes to `results/live-<timestamp>/`. The usual five CSVs show the automatic ten-probe validation. `LIVE_MESSAGES.csv` separately records your messages, sequence numbers, exact frame bytes, observed reception at each host, capture timestamps and PASS/FAIL. A failed or interrupted session also has `ERROR.txt`; it must not be presented as a completed successful run. Namespaces and switch processes are cleaned up on completion, timeout or `/quit`.

The automatic `run_demo.py` remains available without input prompts. Run only one demo at a time.
